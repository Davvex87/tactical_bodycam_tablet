...
SWEP.Purpose = "RMB – Attach camera | R – Open tablet | E + LMB - Take off camera"
...
function SWEP:SecondaryAttack()
    ...
    self:SetNWFloat("HoldingTime", CurTime() + GetConVar("bodycam_time_to_attach"):GetInt())
    self:SetNWInt("bodycam_time_to_attach_cl", GetConVar("bodycam_time_to_attach"):GetInt())

    timer.Create("SWEPHOLDING" .. self:EntIndex(), GetConVar("bodycam_time_to_attach"):GetInt(), 1, function()
...
function SWEP:PrimaryAttack()
    if not self.Owner:KeyDown(IN_USE) then return end
    ...
    if not IsValid(tr:GetNWEntity("BodyCam")) then return end
    if tr:GetNWEntity("BodyCam") ~= self then self.Owner:ChatPrint("You're not Bodycam owner") return end

    tr:SetNWEntity("BodyCam",NULL)
    table.RemoveByValue(self.BodyCamPeople, tr)

    self.Owner:EmitSound("buttons/button15.wav")
    self.Owner:ChatPrint("Bodycam removed")
end
...
if SERVER then
    hook.Add("SetupPlayerVisibility", "AddRTCamera", function(ply)
        local target = ply:GetNWEntity("Bodycam_SelectedTarget")
        if not IsValid(target) then return end
    ...
    local function RemoveCamera(ent)
        if not IsValid(ent) then return end
        local watcher_wpn = ent:GetNWEntity("BodyCam")
            if not IsValid(watcher_wpn) then return end
                
            table.RemoveByValue(watcher_wpn.BodyCamPeople,ent)
    ...
    hook.Add("PlayerDeath", "BodyCam_Death", function(ply)
        RemoveCamera(ply)
    end)

    hook.Add("OnNPCKilled", "BodyCam_NPC_Death", function(npc)
        RemoveCamera(npc)
    end)

    if DarkRP then
        hook.Add("OnPlayerChangedTeam", "BodyCam_ChangeTeam", function(ply)
            RemoveCamera(ply)
        end)
    end
end
...
if SERVER then util.AddNetworkString("Frame_Remove") end

net.Receive("Frame_Remove", function(len, ply)
    local wep = ply:GetActiveWeapon()
    if IsValid(wep) and wep:GetClass() == "bodycam_tablet" then
        wep:Reload()
    end
end)
...
if CLIENT then
        ...
        function Dermas()
            if IsValid(frejmus.frame) then return end
                ...
                if target_index > max_index then
                    target_index = tonumber(target_index) or 1
                end

                local target = target_tbl[target_index]

                if not IsValid(target) then 
                    table.RemoveByValue(target_tbl, target)
            ...
            frame.Paint = function(self, w, h)
                ...
                draw.SimpleText( target and "" or "No Connection..", "Antek4", w * 0.5, h * 0.5, Color(228,228,228), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
            end
            ...
            if IsValid(target) then
                ...
                local function CreateArrowButton(dir, xPos, iconPath)
                    ...
                    btn.DoClick = function()
                        target_index = math.Clamp(target_index + (dir == "right" and 1 or -1), 1, last_index)
                        frame:Remove()
            ...
            comp_name.Paint = function(me,w,h) 
                draw.SimpleText("Antke Inc. Version 1.7.101", "CameraFont", 10, h - 10, Color(200, 200, 200, 180), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM)
            end
    ...
    net.Receive("bodycam_send_initial_cvars", function()
        _ = net.ReadInt(8)
        _ = net.ReadInt(8)
        initialCameraUp = net.ReadInt(8)
        initialCameraForward = net.ReadInt(8)
    end)
    ...
    hook.Add("PostDrawOpaqueRenderables", "manual_model_draw_example", function()
        for _, ent in ipairs(ents.GetAll()) do
            ...
            local attach_id = ent:LookupBone(bone)
            ...
            pos = pos + (ang:Right() * (-9 + (-1 * (GetConVar("bodycam_camera_forward"):GetInt() or 0))))
            pos = pos + (ang:Forward() * (GetConVar("bodycam_camera_up"):GetInt() or 0))
    ...
    hook.Add("PostDrawHUD", "DrawBodyCamAndHelloWorld", function()
        local ply = LocalPlayer()
            if not IsValid(ply) then return end
        local bodycam = ply:GetNWEntity("BodyCam")
            if not IsValid(bodycam) then return end

        local iconW, iconH = 64, 64
        local x, y = ScrW() - iconW - 10, 10

        local mat = Material("vgui/observation.png")

            local selected_target = ply:GetNWEntity("Client_SelectedTarget")

            if selected_target == ply then
